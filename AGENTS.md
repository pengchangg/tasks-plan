...
**Router / middleware.** `requestLog` (security headers + one access line) wraps `recoverPanic` (a handler panic becomes a logged 500 instead of a dead connection), which wraps the mux; `/health/live`, `/health/ready` sit outside the API; `r.Route("/api/v1", ...)` applies `sameOrigin`; the public auth routes add `s.authLimit` (per-address request brake); a nested group applies `authenticate`; role gating is `s.parentOnly(h)` or an inline actor check. No timeout middleware and no CSRF token — CSRF rests on `sameOrigin` (rejects `Sec-Fetch-Site: cross-site` or mismatched `Origin` host → 403) plus `SameSite=Lax`.
...
- Auth is throttled on two independent budgets (`internal/server/ratelimit.go`): `authIP` (60 burst / 1 per second per client address, applied by `s.authLimit`) protects the process; `authFailures` (10 burst / 1 per 30 s per family actor) protects the secret. The failure budget is charged ONLY after a wrong credential, so a correct password/PIN is never locked out — do not move that check before verification, or knowing a public family code becomes a denial-of-service against the family.
...
Demo credentials (`serve --demo`): family `DEMO`, parent `parent` / `growjoy2468`, child PIN `2468`. Two pinning traps: the server serves whatever currently sits in `dist/` (editing `src/` requires `npm run build`; no HMR through Go), and the dev proxy hardcodes port 8080 — moving `GROWJOY_ADDR` requires editing `vite.config.mjs` too. `check:*` scripts are immune (ephemeral port via `BASE_URL`).
...
- Auth: argon2id PHC hashes (`hashSecret`/`verifySecret`, cost re-parsed from the stored string) reached through the gated `s.hashPassword`/`s.verifyPassword`; sessions persist only `tokenHash(raw)` = sha256 hex; cookie `growjoy_session` (`HttpOnly`, `SameSite=Lax`, `Secure=GROWJOY_SECURE_COOKIES`), 12h parent / 30d child. Child PIN must be exactly 4 digits (validated in `saveChild`). No session GC on the request path — `s.Sweep(ctx)` deletes expired sessions and idempotency keys older than `idempotencyRetention` (30d), is called on boot and hourly by `serve`, and also evicts idle limiter buckets.
- Credential budgets are keyed per family actor via `authFailureKey(familyCode, "parent:"+username | "child:"+id)`; keys are only built for rows that exist, so client input cannot mint unbounded buckets. `GET /api/v1/auth/profiles` is unauthenticated by design and is therefore protected by the per-address limiter only.
...
- All application state lives in `useAppStore()` (`src/store.ts`) and is passed down as a `store` prop. No context, no Redux/Zustand, no per-page fetching. Add queries as `actions` entries on the returned object and mutate through `run(() => ...)`, which refreshes server state.
...
| File | Why it matters |
| --- | --- |
| `internal/server/server.go` | `Open`/`applyMigrations`, `Handler` (full route map), `state`/`ensureInstances`/`streaks`/`stats`, `submitTask`/`reviewTask`/`redeemWish` (the pattern to copy for any new mutation), `media`/`inspectUpload`, DTOs. |
...
| `src/store.ts` | The single state container and API client. |
...
**Go — `go test ./...` (or `npm run test:go`).** Single test file `internal/server/server_test.go`: 10 tests, 5 helpers, stdlib `testing` only (no testify/sqlmock — real SQLite file per test in `t.TempDir()`), white-box `package server`, driven through `s.Handler()` + `httptest.NewRecorder()` (no listener, no ports).

- Reuse the existing helpers instead of new scaffolding: `testServer(t)` (temp dir + `Open` with a discard `slog` handler + `t.Cleanup(s.Close)` + DEMO family + `SeedDemo`), `doJSON`, `parentCookie`, `getStateTest`, `multipartRequest`.
- Hardening coverage to extend rather than duplicate: `TestAuthFailureBudgetThrottlesGuessingWithoutLockingOutTheFamily` (10 wrong PINs then 429, correct PIN still 200, budgets are per actor and per family), `TestSaturatedHashGateShedsLoad` (503 + no session), `TestSweepReclaimsExpiredSessionsAndStaleKeys`, `TestInternalErrorsAreLoggedNotLeakedAndPanicsAreRecovered` (asserts the client body never carries the underlying error while the logger does; use a `bytes.Buffer` slog handler).
...
- Covered: migrations applied once across restart, cross-origin rejection, timezone-aware weekly/daily scheduling + streaks, upload content sniffing, concurrent-submit idempotency, exactly-once review credit + minimum growth, redemption overdraw guard, family isolation, last-child protection. Untested: the CLI, health endpoints, `spaHandler`, most CRUD handlers, media download, stats.
